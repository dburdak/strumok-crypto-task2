...
Атака часткового вгадування (Guess-and-Determine) є методом криптоаналізу, який дозволяє зменшити складність перебору внутрішнього стану потокового шифру за рахунок використання залежностей між змінними.

Ідея полягає в тому, що замість повного перебору всього внутрішнього стану, обирається підмножина змінних (базис), значення яких вгадуються. Після цього інші змінні можуть бути обчислені детерміновано через систему рівнянь, що описує роботу шифру.

Для шифру Струмок внутрішній стан формується регістрами LFSR та FSM, між якими існують нелінійні та лінійні залежності. Ці залежності можна представити у вигляді системи рівнянь над бітовими змінними.

Таким чином, атака складається з трьох етапів:
* Побудова моделі залежностей внутрішнього стану.
* Вибір базису (змінних, які будуть вгадуватись).
* Обчислення інших змінних через побудовану систему.

Ефективність атаки визначається розміром базису: чим він менший, тим менша складність атаки. Для Струмок-512 відомо, що існують базиси, які знижують складність до приблизно 2^448.
